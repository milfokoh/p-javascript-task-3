...
function preExists(object, duration) {
    ...
    for (const [key, value] of Object.entries(object)) {
        if (value.startBeforeWork == duration) {
            ...
            mapSchedule.set(countMap, [key, value.startBeforeWork, time].join(" "));
            countMap++;
        }  
        if (value.startAfterWork == duration) {
            ...
            time = `${parseInt(time / 60)}:${(time % 60).toString().padStart(2, '0')}`;
            mapSchedule.set(countMap, [key, value.startAfterWork, time].join(" "));
            countMap++;
        }
        if (value.startBeforeWork > duration) {
            let time = value.startWork;
        ...
        if (value.startAfterWork > duration) {
            ...
            time = parseInt(time.slice(0,2) * 60 + time.slice(3,5) * 1);
            time = `${parseInt(time / 60)}:${(time % 60).toString().padStart(2, '0')}`;
            mapSchedule.set(countMap, [key, duration, time].join(" "));
            countMap++;
            mapSchedule.set(countMap, [key, duration, differenceTime].join(" "));
            countMap++;            
        }
    }
...
function getAppropriateMoment(schedule, duration, workingHours) {
    ...
    return {
         ...
         */
        exists: function () {            
            if (mapSchedule.size !== 0) return true;
            else return false;
        },
        ...
        format: function (template) {
            let array = Array.from(mapSchedule.values());
            let value = array[count];
            if (mapSchedule.size !== count) {                
         ...
         */
        tryLater: function () {
            if (mapSchedule.get(count + 1) !== undefined) {
                count ++;
                console.info(true);
            }
